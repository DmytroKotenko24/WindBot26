...
try:
    ...
    image_part_pre_tratT=np.zeros((720, 1280), dtype=np.uint16) # Inicializando uma imagem de pré-processamento de profundidade com zeros
    ...
    for i in tqdm(range(nr_frames_a_considerar)):
        ...
        filtered_depthT = filter_temporal.process(depthT) # Aplica o filtro temporal para suavizar o fluxo de profundidade
        depth_imageT = np.asanyarray(filtered_depthT.get_data()) # Converte os dados de profundidade filtrados para um array do Numpy
    ...
    for lT in tqdm(range (1280)):  # Precorre cada coluna da imagem (1280 colunas)
        for kT in range(720):  # Precorre cada linha da imagem (720 linhas) 
            ...
            if len(zzs_aceptableT) <5:  # If no non-zero values are present
                ...
                disT= [(zT, abs(zT - mediazT)) for zT in zzs_aceptableT] # Calcula a diferença entre os valores de profundidade e a média
                disT.sort(key=lambda xT: xT[1]) #Ordena pela distância da média
                nT = int(round(len(zzs_aceptableT) * 0.7)) # Seleciona os 'n' valores mais próximos da média (70% dos valores)

                image_rearengedT[kT,lT]=np.uint16(round(np.mean([xT[0] for xT in disT]))) # Calcula a média dos valores selecionados para o pixel (k, l)
