...
try:
    ...
    for i in tqdm(range(nr_frames_a_considerar)):
        ...
        # Debug
        depth_color_frame = colorizer.colorize(filtered_depth) # colorizer.colorize() é usado para aplicar um mapa de cores à imagem de profundidade, neste caso o "JET" mapeia os valores de profundidade para cores azul (profundidade mais próxima) e vermelho (profundidade mais distante)
...
maior_contorno_bag = max(contorno_bag, key=cv2.contourArea) # cv2.contourArea calcula a área de cada contorno. max() seleciona o contorno de maior área.
...
if enablePlot:
    ...
    fig = go.Figure(data=[go.Scatter3d(
        x=X, y=Y, z=Z,
        mode='markers',
        marker=dict(size=2, color=Z, colorscale='Inferno', colorbar=dict(title='Profundidade'))
    )])
    fig.update_layout(
        scene=dict(
            xaxis_title='X',
